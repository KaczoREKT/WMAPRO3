...
def cut():
    global image, image2
    # Pobierz wartości z trackbarów dla dolnego i górnego zakresu koloru oraz rozmiaru
    ...
    # Ustaw minimalny rozmiar wycinka na 10, aby uniknąć błędów
    if ksize < 10:
        ksize = 10

    # Wytnij fragment obrazu na podstawie wartości z trackbarów
    image2 = image[low_color:low_color+int((ksize/2)),
                   high_color:high_color+ksize]

    # Wyświetl wycięty fragment obrazu w oknie 'obrazek'
    cv2.imshow('obrazek', image2)

# key b
def sift1():
    global image
    # Konwersja obrazu na skalę szarości do przetwarzania przez SIFT
    gimg = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    
    # Utworzenie obiektu SIFT
    siftobject = cv2.SIFT_create()
    
    # Wykrywanie punktów kluczowych i obliczanie deskryptorów w obrazie w skali szarości
    keypoint, descriptor = siftobject.detectAndCompute(gimg, None)
    
    # Rysowanie wykrytych punktów kluczowych na oryginalnym obrazie
    keypointimage = cv2.drawKeypoints(
        image, keypoint, None, color=(0, 255, 0), 
        flags=cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS
    )
    
    # Wyświetlenie obrazu z punktami kluczowymi w oknie o nazwie 'obrazek'
    cv2.imshow('obrazek', keypointimage)

# key n
def sift2():
    global image, image2
    gimg1 = cv2.cvtColor(image2, cv2.COLOR_BGR2GRAY)  # Konwersja image2 na skalę szarości
    gimg2 = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)  # Konwersja image na skalę szarości
    siftobject = cv2.SIFT_create()  # Utworzenie obiektu SIFT
    keypoints_1, descriptors_1 = siftobject.detectAndCompute(gimg1, None)  # Punkty kluczowe i deskryptory dla image2
    keypoints_2, descriptors_2 = siftobject.detectAndCompute(gimg2, None)  # Punkty kluczowe i deskryptory dla image
    bf = cv2.BFMatcher(cv2.NORM_L1, crossCheck=True)  # Utworzenie obiektu BFMatcher
    matches = bf.match(descriptors_1, descriptors_2)  # Dopasowanie deskryptorów
    matches = sorted(matches, key=lambda x: x.distance)  # Sortowanie dopasowań według odległości
    matched_img = cv2.drawMatches(
        image2, keypoints_1, image, keypoints_2, matches, image, flags=2)  # Rysowanie dopasowań na obrazie
    cv2.imshow('obrazek', matched_img)  # Wyświetlenie obrazu z dopasowaniami
